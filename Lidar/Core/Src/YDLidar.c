...
result_t getHealth(device_health * health, uint32_t timeout) {
	result_t  ans;
	uint8_t  recvPos = 0;
	uint32_t currentTs = time(&start_time);
	...
	while ((time(&current_time) - currentTs) <= timeout) {
		uint8_t currentbyte;
		HAL_UART_Receive(&huart2, &currentbyte, 1, 10);
...
result_t waitScanDot(uint32_t timeout) {
	int recvPos = 0;
	uint32_t startTs = time(&start_time);
	...
	if(package_Sample_Index == 0) {
		recvPos = 0;
		while ((time(&current_time) - startTs) <= timeout) {
			uint8_t currentByte;
			HAL_UART_Receive(&huart2, &currentByte, 1, 10);
			switch (recvPos) {
			case 0:
...
result_t sendCommand(uint8_t cmd, const void * payload, size_t payloadsize) {
	...
	HAL_UART_Transmit(&huart2, (uint8_t *)header, 2, 10);
	if((cmd & LIDAR_CMDFLAG_HAS_PAYLOAD)){
		...
		HAL_UART_Transmit(&huart2, &sizebyte, 1, 10);
		HAL_UART_Transmit(&huart2, (uint8_t *)payload, sizebyte, 10);
